...
class CartController extends Controller
{
    public function checkout(){
        ...
        if(! count($data['cart'])) return redirect()->route('home');
    ...
    public function getDataCart(){
        $cart = session()->get('cart', []);
        $view = view('cart.cart-sidebar', compact('cart'))->render();
    ...
    public function addToCart(Request $request)
    {
        ...
        return response()->json(['cart' => $cart, 'view' => $view]);
    }
    ...
    public function update(Request $request)
    {
        if($request->id && $request->quantity){
            ...
            if($request->view && $request->view == 'list') {
                $view = view('cart.cart-list', compact('cart'))->render();
            } else {
                $view = view('cart.cart-sidebar', compact('cart'))->render();
    ...
    public function remove(Request $request)
    {
        if($request->id) {
            ...
            if($request->view && $request->view == 'list') {
                ...
                if(! count($cart)) {
                    $view = view('cart.cart-empty')->render();
                }
